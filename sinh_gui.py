...
class SinhInputError(SinhError):
    ...
    def __init__(self, raw):
        super().__init__(
            "'{0}' is not a real number. Please enter a value like "
            "2, -0.5, or 3.14.".format(raw)
        )
...
class SinhOverflowError(SinhError):
    ...
    def __init__(self, x):
        super().__init__(
            "sinh({0}) is too large to represent on this computer. "
            "Try a value between about -710 and 710.".format(x)
        )
...
class SinhApp:
    ...
    def compute(self):
        ...
        try:
            ...
            return
        self.result.config(text="sinh({0}) = {1}".format(x, value),
                           fg="#0F4C5C")
